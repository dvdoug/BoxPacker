...
class VolumePacker implements LoggerAwareInterface
{
    ...
    private function packRotation(int $boxWidth, int $boxLength): PackedBox
    {
        ...
        while ($items->count() > 0) {
            ...
            $preliminaryLayer = $this->layerPacker->packLayer($preliminaryItems, clone $packedItemList, $layerStartDepth, $boxWidth, $boxLength, $this->box->getInnerDepth() - $layerStartDepth, 0, true);
            ...
            if ($preliminaryLayer->getDepth() === $preliminaryLayer->getItems()[0]->getDepth()) { // preliminary === final
                ...
                $layers[] = $this->layerPacker->packLayer($items, $packedItemList, $layerStartDepth, $boxWidth, $boxLength, $this->box->getInnerDepth() - $layerStartDepth, $preliminaryLayer->getDepth(), true);
            }
        }
        ...
        if (!$this->singlePassMode && $layers) {
            ...
            $layers[] = $this->layerPacker->packLayer($items, $this->getPackedItemList($layers), 0, $boxWidth - $maxLayerWidth, $boxLength, $this->box->getInnerDepth(), $this->box->getInnerDepth(), false);

            $maxLayerLength = max(array_map(static function (PackedLayer $layer) {
                return $layer->getEndY();
            }, $layers));
            $layers[] = $this->layerPacker->packLayer($items, $this->getPackedItemList($layers), 0, $boxWidth, $boxLength - $maxLayerLength, $this->box->getInnerDepth(), $this->box->getInnerDepth(), false);
        }
